...
bind(){
  ...
  // Resize observer — перерисовываем ruler/grid при изменении размера окна
  ...
  if(this.drag.active){
    ...
    if(this.drag.type==='handle'){
      ...
      if(rp&&mode==='roll'){
        ...
        rp.b.end=Math.max(rp.b.start+.02, rp.b.start+(totalDur-(rp.a.end-rp.a.start)));
      }else{
...
bindInspectorSingle(tr,it){
  ...
  ['ins-start','ins-end','ins-text'].forEach(id=>{const el=ins(id);if(el){el.onkeydown=ev=>{if(ev.key==='Enter'){ev.preventDefault();applyFn()}}}});
...
saveAutoDraft(key){
  try{
    ...
    const mainKey=this.DRAFT_KEY;
    // rolling timestamped key для recent list (меняется каждые 5 минут)
    ...
    localStorage.setItem(mainKey,JSON.stringify(snap));  // всегда для restore-on-launch
    localStorage.setItem(rKey,JSON.stringify(snap));      // timestamped для recent list
