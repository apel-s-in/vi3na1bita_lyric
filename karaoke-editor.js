...
bind(){
  ...
  // Validate / Restore / Help / Hotkeys / Recent
  this.ui.btnValidate.onclick=()=>this.openValidationModal();
  this.ui.btnRestoreSession.onclick=()=>this.openRecentModals();
  ...
  this.ui.btnValidationRerun.onclick=()=>this.openValidationModal();
  ...
  this.ui.btnStop.onclick=()=>{this.audioElement.pause();this.audioElement.currentTime=0;this.syncPlayhead();this.ui.btnPlay.textContent='▶ Play'};
  ...
  document.addEventListener('click',e=>{
    ...
    const modals=['hotkeysModal','validationModal','helpModal','renameModal','recentModal','loopModal'];
    modals.forEach(m=>{if(e.target===this.ui[m])this.ui[m].classList.add('hidden')});
  });
  this.setupContext();
...
normalizeTrack(raw,forceType,name){
  const t={id:'T_'+this.uid(),type:forceType,name:name||forceType,visible:true,solo:false,locked:false,editable:true,items:[]};
  if(forceType==='line'){
...
rulerStep(){
  ...
  if(pxPerSec>=200)return .5;
  if(pxPerSec>=80)return 1;
  if(pxPerSec>=30)return 2;
  if(pxPerSec>=12)return 5;
  return 10;
},
...
toggleCollapse(id){
  ...
  this.renderHeaders();this.renderTimeline();
},
...
renderTimeline(){
  ...
  this.project.tracks.forEach((tr,ti)=>{
    ...
    const items=this.visibleItems(tr,layerM);

    items.forEach(it=>{
      const px=it.start*this.zoom,pw=(it.end-it.start)*this.zoom,
            isSel=this.selected.trackId===tr.id&&this.selected.ids.has(it.id);
      const div=document.createElement('div');
      div.className='track-item item-'+it.kind+(isSel?' selected':'')+(it.kind==='line'&&tr.type==='words'?'':'')+this.conflictClass(tr,it);
      ...
      div.title=it.text;
      div.dataset.id=it.id;div.dataset.tid=tr.id;
      ...
      if(isActive){
        ...
        div.append(lh2,rh);
        div.addEventListener('mousedown',e=>{if(!e.target.classList.contains('handle'))this.startItemDrag(e,tr.id,it.id)});
      }
...
conflictClass(tr,it){
  const others=tr.items.filter(x=>x.id!==it.id&&x.kind===it.kind);
  const ov=others.some(x=>x.start<it.end&&x.end>it.start);
  return ov?' conflict':'';
},
...
/* ─── playhead ───────────────────────────────────────────────────── */
syncPlayhead(){
  const t=this.audioElement.currentTime||0,px=t*this.zoom,w=this.ui.scrollArea.scrollWidth;
  this.ui.playhead.style.left=px+'px';
  const m=this.duration||1;
  this.ui.timeDisplay.textContent=this.fmtTime(t)+' / '+this.fmtTime(m);
...
updatePlayingHighlights(t){
  ...
  if(!this.project.tracks.length)return;
...
centerOnPlayhead(){
  const t=this.audioElement.currentTime,px=t*this.zoom,sw=this.ui.timelineContainer.clientWidth;
...
handleTimelineMouseDown(e){
  ...
  if(handle){
    ...
    let rollPair=null;
    if(mode==='roll'&&side==='right'){const nx=this.nextItem(tr,it);if(nx)rollPair={a:it,b:nx}}
    if(mode==='roll'&&side==='left'){const pr=this.prevItem(tr,it);if(pr)rollPair={a:pr,b:it}}
  ...
  if(item){
    ...
    if(!e.shiftKey&&!(this.selected.trackId===tid&&this.selected.ids.has(id))){
      this.clearSelection();
      this.selectItem(tid,id);
    }else if(e.shiftKey){this.selectItem(tid,id)}
    const selSnap=this.selected.trackId===tid?[...this.selected.ids].map(sid=>({item:this.itemById(tid,sid),origStart:this.itemById(tid,sid).start,origEnd:this.itemById(tid,sid).end})).filter(x=>x.item):[{item:it,origStart:it.start,origEnd:it.end}];
    this.drag={active:true,type:'move',trackId:tid,itemId:id,startX:e.clientX,
      initial:{start:it.start,end:it.end},sel:selSnap,mode:this.ui.dragMode.value};
    return;
  }
  ...
  if(this.drag.active){
    ...
    if(this.drag.type==='move'){
      ...
      this.drag.sel.forEach(s=>{
        minDelta=Math.max(minDelta,-s.origStart);
      });
      let rawDelta=this.snapStep?Math.round(dx/this.snapStep)*this.snapStep:dx;
      rawDelta=Math.max(rawDelta,minDelta);

      if(mode!=='free'){
        // Ripple batch: compute valid delta against non-selected neighbours
        const selIds=new Set(this.drag.sel.map(s=>s.item.id));
        const others=tr.items.filter(i=>!selIds.has(i.id));
        this.drag.sel.forEach(s=>{
          const newS=s.origStart+rawDelta,newE=s.origEnd+rawDelta;
          others.forEach(o=>{
            if(o.kind!==s.item.kind)return;
            if(rawDelta>0&&o.start>=s.origEnd-1e-9){const lim=o.start-(newE-s.origEnd);maxDelta=Math.min(maxDelta,lim);rawDelta=Math.min(rawDelta,maxDelta)}
            if(rawDelta<0&&o.end<=s.origStart+1e-9){const lim=o.end-s.origStart;minDelta=Math.max(minDelta,lim);rawDelta=Math.max(rawDelta,minDelta)}
          });
        });
      }
      const finalDelta=rawDelta;
      this.drag.sel.forEach(s=>{s.item.start=Math.max(0,s.origStart+finalDelta);s.item.end=Math.max(s.item.start+.02,s.origEnd+finalDelta)});
    }
    ...
    if(this.drag.type==='handle'){
      ...
      if(rp&&mode==='roll'){
        const totalDur=rp.b.end-rp.a.start;
        let newMid=Math.max(rp.a.start+.02,Math.min(rp.b.end-.02,(this.drag.side==='right'?rp.a.start:rp.b.end)+(dx)));
        if(this.snapStep)newMid=Math.round(newMid/this.snapStep)*this.snapStep;
        rp.a.end=newMid;rp.b.start=newMid;
        // preserve total duration
        const usedDur=rp.a.end-rp.a.start+(rp.b.end-rp.b.start);
        if(Math.abs(usedDur-totalDur)>.001)rp.b.end=rp.a.start+totalDur-(rp.a.end-rp.a.start)+rp.b.start-rp.b.start;
      }else{
        ...
        if(this.drag.side==='left'){
          raw=Math.max(0,Math.min(raw,it.end-.02));
          if(mode!=='free'){const pr=this.prevItem(tr,it);if(pr&&pr.kind===it.kind)raw=Math.max(raw,pr.end)}
          it.start=raw;
        }else{
          raw=Math.max(it.start+.02,raw);
          if(mode!=='free'){const nx=this.nextItem(tr,it);if(nx&&nx.kind===it.kind)raw=Math.min(raw,nx.start)}
          it.end=raw;
          if(mode==='ripple'){this.rippleRight(tr,it,raw)}
        }
      }
    }

    this.renderTimeline();this.renderInspector();this.renderPreview();return;
  }
...
handleGlobalMouseUp(){
  if(this.playheadDrag.active){
    this.playheadDrag.active=false;document.body.style.cursor='';return;
  }
  if(this.charDrag.active){this.commitCharDrag();return}
  if(this.drag.active){
    this.resolveNoOverlap();
  ...
  if(this.resize.active){this.resize.active=false;document.body.style.cursor='';this.persistUiPrefs()}
},
...
rippleRight(tr,anchor,newEnd){
  const delta=newEnd-this.drag.initial.end;if(Math.abs(delta)<1e-6)return;
  const others=tr.items.filter(i=>i.kind===anchor.kind&&i.start>=anchor.end-1e-9&&i.id!==anchor.id);
  others.forEach(o=>{o.start=Math.max(0,o.start+delta);o.end=Math.max(o.start+.02,o.end+delta)});
},

resolveNoOverlap(){
  const mode=this.ui.dragMode.value;if(mode==='free')return;
  const tr=this.activeTrack();if(!tr)return;
  ['line','word','char'].forEach(kind=>{
    const items=tr.items.filter(i=>i.kind===kind);
    this.sortItems(items);
    for(let i=1;i<items.length;i++){
      if(items[i].start<items[i-1].end){
        if(mode==='ripple'){
          const d=items[i-1].end-items[i].start;
          items[i].start+=d;items[i].end+=d;
        }else{
...
prevItem(tr,it){
  const same=tr.items.filter(i=>i.kind===it.kind).sort((a,b)=>a.start-b.start);
  const idx=same.findIndex(i=>i.id===it.id);
...
bindInspectorSingle(tr,it){
  ...
  const applyFn=()=>{
    ...
    it.start=Math.max(0,ns);it.end=Math.max(it.start+.02,ne);
    it.text=ins('ins-text').value;
    this.markDirty();this.sortActiveTrack();this.renderTimeline();this.renderInspector();this.renderPreview();
  };
  ins('ins-apply').onclick=applyFn;
  ['ins-start','ins-end','ins-text'].forEach(id=>{const el=ins(id);if(el){el.onkeydown=ev=>{if(ev.key==='Enter')applyFn()};el.onblur=applyFn}});
  ins('ins-split')?.addEventListener('click',()=>this.splitAtPlayhead());
  ins('ins-merge-prev')?.addEventListener('click',()=>this.mergeSelectedWithPrev());
...
handleCharDragMove(e){
  ...
  const durPx=(it.end-it.start)*this.zoom;
  const pct=(e.clientX-startX)/Math.max(1,durPx);
  const dt=pct*(it.end-it.start);
  const tl=document.getElementById('chars-tl');if(!tl)return;
  const width=tl.offsetWidth||200;
  const dxPx=e.clientX-startX,dtBig=dxPx/width*(it.end-it.start);
  if(side==='left'){
    ch.start=Math.max(it.start,Math.min(initialStart+dtBig,this.num(ch.end,it.end)-.02));
  }else{
    ch.end=Math.max(this.num(ch.start,it.start)+.02,Math.min(initialEnd+dtBig,it.end));
  }
...
commitCharDrag(){
  ...
  const tr=this.trackById(this.charDrag.trackId),it=this.itemById(this.charDrag.trackId,this.charDrag.itemId);
  if(tr&&it)this.renderInspector();
},
...
bindInspectorBatch(tr,items){
  document.getElementById('batch-move')?.addEventListener('click',()=>{
    ...
    this.markDirty();this.renderTimeline();this.renderInspector();this.renderPreview();
  });
  document.getElementById('batch-norm')?.addEventListener('click',()=>{
    ...
    this.markDirty();this.renderTimeline();this.renderInspector();
  });
  document.getElementById('batch-close')?.addEventListener('click',()=>{
    ...
    sorted.forEach((it,i)=>{if(i>0){it.start=sorted[i-1].end+gap;it.end=Math.max(it.start+.02,it.end)}});
    this.markDirty();this.renderTimeline();this.renderInspector();
  });
  document.getElementById('batch-dist')?.addEventListener('click',()=>this.distributeItemsEvenly(items));
  document.getElementById('ins-del')?.addEventListener('click',()=>this.deleteSelected());
},
...
batchMoveRipple(tr,selItems,delta){
  ...
  const others=tr.items.filter(i=>!selIds.has(i.id));
  // clamp delta so nothing goes below 0
  let fd=delta;
  selItems.forEach(i=>{fd=Math.max(fd,-i.start)});
  if(this.ui.dragMode.value!=='free'){
    // ripple: also move items after selection if they'd overlap
    selItems.forEach(i=>{
  ...
  selItems.forEach(i=>{i.start=Math.max(0,i.start+fd);i.end=Math.max(i.start+.02,i.end+fd)});
  this.sortTrack(tr);
},
...
splitAtPlayhead(){
  ...
  ids.forEach(id=>{
    ...
    const newIt={...it,id:(it.kind==='word'?'W_':'L_')+this.uid(),start:t,chars:[]};
    it.end=t;it.chars=(it.chars||[]).filter(c=>this.num(c.start,it.start)<t);
    tr.items.push(newIt);
  });
...
splitAtCursor(cursorT){
  const tr=this.activeTrack();if(!tr)return;
  if(!this.selected.ids.size)return;
  const ids=[...this.selected.ids];
  this.pushHistory('Split at Cursor');
  ids.forEach(id=>{
    const it=this.itemById(tr.id,id);
    if(!it||cursorT<=it.start||cursorT>=it.end)return;
    const nit={...it,id:(it.kind==='word'?'W_':'L_')+this.uid(),start:cursorT,chars:[]};
    it.end=cursorT;tr.items.push(nit);
  });
  this.sortTrack(tr);this.markDirty();this.renderTimeline();this.renderInspector();this.renderPreview();
},
...
mergeSelectedWithNext(){
  ...
  ids.forEach(id=>{
    ...
    const nx=this.nextItem(tr,it);if(!nx)return;
    it.end=nx.end;it.text+=' '+nx.text;
...
mergeSelectedWithPrev(){
  ...
  ids.forEach(id=>{
    const it=this.itemById(tr.id,id);if(!it)return;
    const pr=this.prevItem(tr,it);if(!pr)return;
    pr.end=it.end;pr.text+=' '+it.text;
...
duplicateSelected(){
  ...
  ids.forEach(id=>{
    const it=this.itemById(tr.id,id);if(!it)return;
    const dur=it.end-it.start;
    const nx=this.nextItem(tr,it);
    const newStart=nx?Math.max(it.end,nx.start-dur):it.end;
    const nit={...it,id:(it.kind==='word'?'W_':'L_')+this.uid(),start:newStart,end:newStart+dur,chars:JSON.parse(JSON.stringify(it.chars||[]))};
...
zoomToSelection(){
  ...
  const minS=Math.min(...items.map(i=>i.start)),maxE=Math.max(...items.map(i=>i.end));
  const w=this.ui.timelineContainer.clientWidth;
  const newZ=Math.max(10,Math.floor(w/(maxE-minS)*.85));
  this.setZoom(newZ,false);
  setTimeout(()=>this.ui.timelineContainer.scrollLeft=minS*this.zoom-20,50);
},
...
scrollToSelection(){
  ...
  const w=this.ui.timelineContainer.clientWidth;
  this.ui.timelineContainer.scrollLeft=Math.max(0,minS*this.zoom-w/2);
},
...
gotoSelectionStart(){
  ...
  if(!items.length)return;
  const minS=Math.min(...items.map(i=>i.start));
  this.audioElement.currentTime=minS;this.syncPlayhead();
},
...
showContextMenu(e){
  e.preventDefault();
  const scrollLeft=this.ui.timelineContainer.scrollLeft,scrollTop=this.ui.timelineContainer.scrollTop;
  const rect=this.ui.scrollArea.getBoundingClientRect();
...
setupContext(){
  this.ui.ctxEdit.onclick=()=>{this.ui.contextMenu.classList.add('hidden');const it=this.itemById(this.context.trackId,this.context.itemId);if(!it)return;const t=prompt('Текст:',it.text);if(t!==null){this.pushHistory('Edit Text');it.text=t;this.markDirty();this.renderTimeline();this.renderInspector();this.renderPreview()}};
  this.ui.ctxSplit.onclick=()=>{this.ui.contextMenu.classList.add('hidden');this.splitAtPlayhead()};
  this.ui.ctxSplitCursor.onclick=()=>{this.ui.contextMenu.classList.add('hidden');this.splitAtCursor(this.context.cursorX)};
  this.ui.ctxMergePrev.onclick=()=>{this.ui.contextMenu.classList.add('hidden');this.mergeSelectedWithPrev()};
  this.ui.ctxMergeNext.onclick=()=>{this.ui.contextMenu.classList.add('hidden');this.mergeSelectedWithNext()};
  this.ui.ctxDuplicate.onclick=()=>{this.ui.contextMenu.classList.add('hidden');this.duplicateSelected()};
  this.ui.ctxAddLine.onclick=()=>{this.ui.contextMenu.classList.add('hidden');this.addNewLine()};
  this.ui.ctxAddWord.onclick=()=>{this.ui.contextMenu.classList.add('hidden');this.addNewWord()};
  this.ui.ctxBatchClose.onclick=()=>{this.ui.contextMenu.classList.add('hidden');this.batchCloseGapsCtx()};
  this.ui.ctxBatchDist.onclick=()=>{this.ui.contextMenu.classList.add('hidden');const tr=this.activeTrack();if(tr&&this.selected.ids.size>1){const items=[...this.selected.ids].map(id=>this.itemById(tr.id,id)).filter(Boolean);this.distributeItemsEvenly(items)}};
  this.ui.ctxBatchNorm.onclick=()=>{this.ui.contextMenu.classList.add('hidden');const dur=this.num(prompt('Длительность (s):','1.0'),0);if(dur>0){const tr=this.activeTrack();if(!tr)return;this.pushHistory('Batch Norm');[...this.selected.ids].forEach(id=>{const it=this.itemById(tr.id,id);if(it)it.end=it.start+dur});this.markDirty();this.renderTimeline();this.renderInspector()}};
  this.ui.ctxMovePlayhead.onclick=()=>{this.ui.contextMenu.classList.add('hidden');const tr=this.activeTrack();if(!tr||!this.selected.ids.size)return;const items=[...this.selected.ids].map(id=>this.itemById(tr.id,id)).filter(Boolean);if(!items.length)return;const t=this.audioElement.currentTime,minS=Math.min(...items.map(i=>i.start)),delta=t-minS;this.pushHistory('Move to Playhead');items.forEach(i=>{i.start+=delta;i.end+=delta});this.markDirty();this.renderTimeline();this.renderInspector()};
...
batchCloseGapsCtx(){
  ...
  const sorted=items.sort((a,b)=>a.start-b.start);
  this.pushHistory('Batch Close Gaps');
  sorted.forEach((it,i)=>{if(i>0){it.start=sorted[i-1].end;it.end=Math.max(it.start+.02,it.end)}});
  this.markDirty();this.renderTimeline();this.renderInspector();
},
...
setZoom(v,keepPlayhead){
  ...
  this.updateZoomReadout();
  if(keepPlayhead){
    const t=this.audioElement.currentTime,sc=this.ui.timelineContainer,w=sc.clientWidth;
...
renderPreview(){
  ...
  if(source===wordsTr&&!lineTr){
    const lines=wordsTr.items.filter(i=>i.kind==='line').sort((a,b)=>a.start-b.start);
    lines.forEach(line=>{
      const words=wordsTr.items.filter(i=>i.kind==='word'&&i.lineId===line.id).sort((a,b)=>a.start-b.start);
      const playing=t>=line.start&&t<line.end;
      const selUI=this.selected.trackId===wordsTr.id&&this.selected.ids.has(line.id);
      html+=`<div class="lyric-line${playing?' playing':''}${selUI?' selected-ui':''}" data-id="${line.id}" data-tid="${wordsTr.id}">`;
      if(words.length){
        words.forEach(w=>{
          const wp=t>=w.start&&t<w.end;
          html+=`<span class="lyric-word${wp?' playing':''}">${this.esc(w.text)} </span>`;
        });
      }else{html+=this.esc(line.text)}
      html+='</div>';
    });
  }else if(lineTr&&wordsTr){
    // dual track preview
    const lineItems=lineTr.items.sort((a,b)=>a.start-b.start);
    lineItems.forEach(line=>{
      const playing=t>=line.start&&t<line.end;
      const selUI=this.selected.trackId===lineTr.id&&this.selected.ids.has(line.id);
      html+=`<div class="lyric-line preview-subline${playing?' playing':''}${selUI?' selected-ui':''}" data-id="${line.id}" data-tid="${lineTr.id}">`;
      html+=`<span class="preview-underlay">${this.esc(line.text)}</span>`;
      // overlay matching words from wordsTr
      const words=wordsTr.items.filter(i=>i.kind==='word'&&i.start>=line.start&&i.end<=line.end+.1).sort((a,b)=>a.start-b.start);
      if(words.length){
        html+=' <span style="opacity:.7">|</span> ';
        words.forEach(w=>{
          const wp=t>=w.start&&t<w.end;
          html+=`<span class="lyric-word preview-overlay${wp?' playing':''}">${this.esc(w.text)} </span>`;
        });
      }
      html+='</div>';
    });
    ...
    // single line track
    const lines=source.items.filter(i=>i.kind==='line').sort((a,b)=>a.start-b.start);
    lines.forEach(line=>{
      const playing=t>=line.start&&t<line.end;
      const selUI=this.selected.trackId===source.id&&this.selected.ids.has(line.id);
      html+=`<div class="lyric-line${playing?' playing':''}${selUI?' selected-ui':''}" data-id="${line.id}" data-tid="${source.id}">${this.esc(line.text)}</div>`;
    });
  }
  ...
  this.ui.lyricsContainer.querySelectorAll('.lyric-line').forEach(el=>{
    el.onclick=()=>{
      const tid=el.dataset.tid,id=el.dataset.id;
      const tr=this.trackById(tid),it=this.itemById(tid,id);
      if(it){this.audioElement.currentTime=it.start;this.syncPlayhead();this.selectItem(tid,id)}
    };
  });

  // auto-scroll to playing line
  const playing=this.ui.lyricsContainer.querySelector('.lyric-line.playing');
...
exportTrack(tr){
  ...
  try{
    const json=this.serializeTrack(tr);
    const blob=new Blob([JSON.stringify(json,null,2)],{type:'application/json'});
    this.downloadBlob(blob,tr.name+'_export.json');
  }catch(e){alert('Ошибка экспорта: '+e.message)}
},

serializeTrack(tr){
  if(tr.type==='line'){
    return [...tr.items].filter(i=>i.kind==='line').sort((a,b)=>a.start-b.start).map(i=>({time:+i.start.toFixed(3),line:i.text}));
  }
  // words type
  ...
  return lines.map(line=>{
    ...
    return{
      start:+line.start.toFixed(3),
      end:+line.end.toFixed(3),
      line:line.text,
      words:words.map(w=>({
...
exportAll(){
  this.project.tracks.forEach(tr=>this.exportTrack(tr));
},

exportZip(){
  // Pure JS zip via stored files — uses concatenated blob trick (not real deflate, store only)
  // For full deflate support user would need JSZip, but we implement store-only .zip
  try{
    const files=this.project.tracks.map(tr=>({
      name:tr.name+'_export.json',
      data:new TextEncoder().encode(JSON.stringify(this.serializeTrack(tr),null,2))
    }));
    if(!files.length){alert('Нет дорожек');return}
    const blob=this.buildZipBlob(files);
    this.downloadBlob(blob,'karaoke_export.zip');
...
buildZipBlob(files){
  ...
  const parts=[],cdirs=[];let offset=0;
  const enc=new TextEncoder();
  files.forEach(f=>{
    const nameB=enc.encode(f.name),data=f.data;
    const crc=crc32(data),cs=data.length;
    const lh=new Uint8Array(30+nameB.length);
    u32le(0x04034b50,lh,0);u16le(20,lh,4);u16le(0,lh,6);u16le(0,lh,8);
    u16le(0,lh,10);u16le(0,lh,12);
    u32le(crc,lh,14);u32le(cs,lh,18);u32le(cs,lh,22);
    u16le(nameB.length,lh,26);u16le(0,lh,28);
    lh.set(nameB,30);
    const cd=new Uint8Array(46+nameB.length);
    u32le(0x02014b50,cd,0);u16le(20,cd,4);u16le(20,cd,6);u16le(0,cd,8);u16le(0,cd,10);
    u16le(0,cd,12);u16le(0,cd,14);
    u32le(crc,cd,16);u32le(cs,cd,20);u32le(cs,cd,24);
    u16le(nameB.length,cd,28);u16le(0,cd,30);u16le(0,cd,32);u16le(0,cd,34);u16le(0,cd,36);
    u32le(0,cd,38);u32le(offset,cd,42);cd.set(nameB,46);
    parts.push(lh,data);cdirs.push(cd);
    offset+=lh.length+cs;
  });
  const cdSize=cdirs.reduce((s,c)=>s+c.length,0);
  const eocd=new Uint8Array(22);
  u32le(0x06054b50,eocd,0);u16le(0,eocd,4);u16le(0,eocd,6);
  u16le(files.length,eocd,8);u16le(files.length,eocd,10);
  u32le(cdSize,eocd,12);u32le(offset,eocd,16);u16le(0,eocd,20);
...
buildSessionSnapshot(){
  return{
    ...
    autosaveIntervalSec:this.autosaveIntervalSec,
    loop:this.loop,
    collapsed:{...this.collapsed},
    project:JSON.parse(JSON.stringify(this.project)),
    playheadTime:this.audioElement.currentTime||0,
    keymap:{...this.keymap},
...
saveAutoDraft(key){
  key=key||this.DRAFT_KEY;
  try{
    const snap=this.buildSessionSnapshot();
    localStorage.setItem(key,JSON.stringify(snap));
    this.addToRecent(key,snap);
...
saveSessionToFile(){
  ...
  this.downloadBlob(blob,'karaoke-session-'+new Date().toISOString().slice(0,19).replace(/:/g,'-')+'.kep');
...
async loadSessionFromFile(file){
  if(!file)return;
  try{
    const txt=await file.text(),snap=JSON.parse(txt);
    this.applySnapshot(snap);
    this.updateSaveStatus('Session loaded',false);
  }catch(e){alert('Ошибка загрузки сессии: '+e.message)}
},

applySnapshot(snap){
  if(!snap||!snap.project)return;
  this.zoom=snap.zoom||80;
  this.verticalZoom=snap.verticalZoom||1;
  this.snapStep=snap.snapStep||0;
  this.autoScroll=snap.autoScroll!==undefined?snap.autoScroll:true;
  this.volume=snap.volume!==undefined?snap.volume:1;
...
restoreDraftFromStorage(key,force){
  try{
    const raw=localStorage.getItem(key||this.DRAFT_KEY);
    if(!raw)return false;
    const snap=JSON.parse(raw);
    if(!force&&!snap.project)return false;
    this.applySnapshot(snap);
    this.updateSaveStatus('Draft restored',false);
    return true;
...
checkDraftOnLaunch(){
  try{
    ...
    if(!raw)return;
    const snap=JSON.parse(raw);
    if(!snap||!snap.project||!snap.project.tracks||!snap.project.tracks.length)return;
...
openRecentModals(){this.openRecentModal()},

openRecentModal(){
  this.renderRecentList();
  this.ui.recentModal.classList.remove('hidden');
},

renderRecentList(){
  const list=this.getRecentList();
  const c=this.ui.recentList;
  ...
  c.innerHTML=list.map((r,i)=>`
    <div class="draft-row">
      ...
      <button class="btn" data-ri="${i}">Restore</button>
      <button class="btn" data-ri-del="${i}">✕</button>
    </div>`).join('');
  c.querySelectorAll('[data-ri]').forEach(btn=>{
    btn.onclick=()=>{
      const r=list[+btn.dataset.ri];
      this._pendingDraftKey=r.key;
      this.ui.recentModal.classList.add('hidden');
      this.restoreDraftFromStorage(r.key,true);
    };
  });
  c.querySelectorAll('[data-ri-del]').forEach(btn=>{
    btn.onclick=()=>{
      const r=list[+btn.dataset.riDel];
      localStorage.removeItem(r.key);
      this.removeFromRecent(r.key);
      this.renderRecentList();
    };
  });
},
...
defaultKeymap(){
  return{
    ...
    'add_line':{key:'Ctrl+Shift+L',desc:'Add new line at playhead',ru:'Добавить новую строку на позиции воспроизведения'},
    'add_word':{key:'Ctrl+Shift+W',desc:'Add new word at playhead',ru:'Добавить новое слово на позиции воспроизведения'},
...
initKeymap(){
  ...
  try{
    ...
    this.keymap={};
    Object.keys(def).forEach(cmd=>{
      this.keymap[cmd]={...def[cmd],...(saved[cmd]||{})};
    });
...
closeHotkeysModal(){
  this.hotkeysWaiting=null;this.ui.hotkeysModal.classList.add('hidden');
},
...
renderHotkeysModal(){
  ...
  if(conflicts.length){
    this.ui.hotkeysConflicts.textContent='⚠ Конфликты: '+conflicts.map(c=>`"${c.keys}" (${c.cmds.join(', ')})`).join('; ');
  }else{this.ui.hotkeysConflicts.textContent=''}
  ...
  this.ui.hotkeysList.innerHTML=rows.map(([cmd,cfg])=>{
    const isConflict=conflictKeys.has(cmd);
    const isWaiting=this.hotkeysWaiting===cmd;
  ...
  this.ui.hotkeysList.querySelectorAll('[data-set]').forEach(btn=>{
    btn.onclick=()=>{
      this.hotkeysWaiting=btn.dataset.set;
      this.renderHotkeysModal();
    };
  });
  this.ui.hotkeysList.querySelectorAll('[data-clear]').forEach(btn=>{
    btn.onclick=()=>{
      const cmd=btn.dataset.clear;
      if(this.keymap[cmd])this.keymap[cmd].key='';
      this.hotkeysWaiting=null;
      this.renderHotkeysModal();
    };
  });
...
handleHotkeys(e){
  ...
  // hotkey capture mode always takes priority
  if(this.hotkeysWaiting){this.captureHotkeyInput(e);return}

  // allow ctrl combos even inside inputs (save, undo, redo, etc.)
  ...
  if(this.matchHotkey('play_pause',e)){e.preventDefault();this.togglePlay();return}
  if(this.matchHotkey('stop',e)){e.preventDefault();this.audioElement.pause();this.audioElement.currentTime=0;this.syncPlayhead();this.ui.btnPlay.textContent='▶ Play';return}
  if(this.matchHotkey('zoom_in',e)||this.matchHotkey('zoom_in_num',e)){e.preventDefault();this.setZoom(this.zoom+10,true);return}
  if(this.matchHotkey('zoom_out',e)||this.matchHotkey('zoom_out_num',e)){e.preventDefault();this.setZoom(this.zoom-10,true);return}
  if(this.matchHotkey('vzoom_in',e)){e.preventDefault();this.setVerticalZoom(this.verticalZoom+.1);return}
  if(this.matchHotkey('vzoom_out',e)){e.preventDefault();this.setVerticalZoom(this.verticalZoom-.1);return}
  if(this.matchHotkey('seek_left',e)){e.preventDefault();this.audioElement.currentTime=Math.max(0,this.audioElement.currentTime-.1);this.syncPlayhead();return}
  if(this.matchHotkey('seek_right',e)){e.preventDefault();this.audioElement.currentTime=Math.min(this.duration||9999,this.audioElement.currentTime+.1);this.syncPlayhead();return}
  if(this.matchHotkey('seek_left_big',e)){e.preventDefault();this.audioElement.currentTime=Math.max(0,this.audioElement.currentTime-1);this.syncPlayhead();return}
  if(this.matchHotkey('seek_right_big',e)){e.preventDefault();this.audioElement.currentTime=Math.min(this.duration||9999,this.audioElement.currentTime+1);this.syncPlayhead();return}
  if(this.matchHotkey('nudge_left',e)){e.preventDefault();this.nudgeSelected(-.05);return}
  if(this.matchHotkey('nudge_right',e)){e.preventDefault();this.nudgeSelected(.05);return}
  if(this.matchHotkey('stretch_left',e)){e.preventDefault();this.stretchSelected('left',-.05);return}
  if(this.matchHotkey('stretch_right',e)){e.preventDefault();this.stretchSelected('right',.05);return}
  if(this.matchHotkey('split',e)){e.preventDefault();this.splitAtPlayhead();return}
  if(this.matchHotkey('merge_next',e)){e.preventDefault();this.mergeSelectedWithNext();return}
  if(this.matchHotkey('merge_prev',e)){e.preventDefault();this.mergeSelectedWithPrev();return}
  if(this.matchHotkey('duplicate',e)){e.preventDefault();this.duplicateSelected();return}
  if(this.matchHotkey('delete',e)||this.matchHotkey('delete_back',e)){e.preventDefault();this.deleteSelected();return}
  if(this.matchHotkey('select_all',e)){e.preventDefault();this.selectAll();return}
  if(this.matchHotkey('deselect',e)){e.preventDefault();this.clearSelection();return}
  if(this.matchHotkey('undo',e)){e.preventDefault();this.undo();return}
  if(this.matchHotkey('redo',e)||this.matchHotkey('redo2',e)){e.preventDefault();this.redo();return}
  if(this.matchHotkey('center_playhead',e)){e.preventDefault();this.centerOnPlayhead();return}
  if(this.matchHotkey('fit_song',e)){e.preventDefault();this.fitSong();return}
  if(this.matchHotkey('zoom_selection',e)){e.preventDefault();this.zoomToSelection();return}
  if(this.matchHotkey('scroll_selection',e)){e.preventDefault();this.scrollToSelection();return}
  if(this.matchHotkey('goto_sel_start',e)){e.preventDefault();this.gotoSelectionStart();return}
  if(this.matchHotkey('toggle_autoscroll',e)){e.preventDefault();this.autoScroll=!this.autoScroll;this.ui.autoScroll.checked=this.autoScroll;return}
  if(this.matchHotkey('toggle_snap',e)){e.preventDefault();const v=this.snapStep?0:.25;this.snapStep=v;this.ui.snapSelect.value=v;return}
  if(this.matchHotkey('save_json',e)){e.preventDefault();this.exportTrack(this.activeTrack());return}
  if(this.matchHotkey('open_hotkeys',e)){e.preventDefault();this.openHotkeysModal();return}
  if(this.matchHotkey('toggle_track',e)){e.preventDefault();this.switchActiveTrack();return}
  if(this.matchHotkey('toggle_line_vis',e)){e.preventDefault();const tr=this.trackByType('line');if(tr)this.toggleTrackProp(tr.id,'visible');return}
  if(this.matchHotkey('toggle_words_vis',e)){e.preventDefault();const tr=this.trackByType('words');if(tr)this.toggleTrackProp(tr.id,'visible');return}
  if(this.matchHotkey('solo_active',e)){e.preventDefault();const tr=this.activeTrack();if(tr)this.toggleTrackProp(tr.id,'solo');return}
  if(this.matchHotkey('lock_active',e)){e.preventDefault();const tr=this.activeTrack();if(tr)this.toggleTrackProp(tr.id,'locked');return}
  if(this.matchHotkey('loop_toggle',e)){e.preventDefault();if(this.loop.enabled)this.clearLoop();else this.openLoopModal();return}
  if(this.matchHotkey('add_line',e)){e.preventDefault();this.addNewLine();return}
  if(this.matchHotkey('add_word',e)){e.preventDefault();this.addNewWord();return}
  if(this.matchHotkey('validate',e)){e.preventDefault();this.openValidationModal();return}
  if(this.matchHotkey('help',e)){e.preventDefault();this.openHelpModal();return}
},
...
nudgeSelected(delta){
  ...
  [...this.selected.ids].forEach(id=>{
    const it=this.itemById(tr.id,id);if(!it)return;
    it.start=Math.max(0,it.start+delta);it.end=Math.max(it.start+.02,it.end+delta);
  });
  this.markDirty();this.renderTimeline();this.renderInspector();
},
...
stretchSelected(side,delta){
  ...
  [...this.selected.ids].forEach(id=>{
    const it=this.itemById(tr.id,id);if(!it)return;
    if(side==='left')it.start=Math.max(0,Math.min(it.end-.02,it.start+delta));
...
/* ─── validation ─────────────────────────────────────────────────── */
openValidationModal(){
  this.ui.validationList.innerHTML=this.runValidation().map(v=>`
    <div class="validation-item ${v.sev}">
      <span class="vi-badge">${v.sev.toUpperCase()}</span>
      <span>${v.msg}</span>
    </div>`).join('')||'<div class="validation-item ok"><span class="vi-badge">OK</span><span>Всё в порядке!</span></div>';
...
runValidation(){
  ...
  this.project.tracks.forEach(tr=>{
    ...
    sorted.forEach((it,i)=>{
      ...
      if(it.kind===it.kind){prevEnd=it.end;prevKind=it.kind}
      if(it.kind==='word'&&tr.type==='words'){
        ...
        (it.chars||[]).forEach((ch,ci)=>{
          if(this.num(ch.start,it.start)<it.start-.01)out.push({sev:'warn',msg:`[${tr.name}] Символ ${ci} "${ch.c}" в слове "${it.text}" начинается раньше слова`});
          if(this.num(ch.end,it.end)>it.end+.01)out.push({sev:'warn',msg:`[${tr.name}] Символ ${ci} "${ch.c}" в слове "${it.text}" заканчивается позже слова`});
        });
      }
    });
...
/* ─── help modal ─────────────────────────────────────────────────── */
openHelpModal(){
  const rows=Object.entries(this.keymap).map(([,cfg])=>`
    <div class="hk-row">
  ...
  this.ui.helpList.innerHTML=rows;
...
startResize(e,target){
  e.preventDefault();
  this.resize={active:true,target,startX:e.clientX,startY:e.clientY,
...
handleResize(e){
  ...
  if(target==='sidebar'){
    const w=Math.max(260,Math.min(820,startDim-(e.clientX-startX)));
