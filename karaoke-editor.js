...
normalizeTrack(raw,forceType,name){
  const t={id:'T_'+this.uid(),type:forceType,name:name||forceType,visible:true,solo:false,locked:false,muted:false,editable:true,items:[]};
  if(forceType==='line'){
...
prevItem(tr,it){return this.getPrevNeighbor(tr,it)},
nextItem(tr,it){return this.getNextNeighbor(tr,it)},
...
applyNoOverlapMove(tr,sel,rawDelta){
  ...
  let d=rawDelta;
  sel.forEach(s=>{
    const newS=s.origStart+d,newE=s.origEnd+d;
    others.forEach(o=>{
      if(o.kind!==s.item.kind)return;
      if(d>0&&o.start>=s.origEnd-1e-9){d=Math.min(d,d-(newE-o.start));d=Math.max(d,0)}
      if(d<0&&o.end<=s.origStart+1e-9){d=Math.max(d,d+(o.end-(s.origStart+d)));d=Math.min(d,0)}
    });
  });
...
renderRecentList(){
  ...
  c.querySelectorAll('[data-ri]').forEach(btn=>{
    btn.onclick=()=>{const r=list[+btn.dataset.ri];this._pendingDraftKey=r.key;this.ui.recentModal.classList.add('hidden');this.restoreDraftFromStorage(r.key,true)};
  });
  c.querySelectorAll('[data-ri-del]').forEach(btn=>{
    btn.onclick=()=>{const r=list[+btn.dataset.riDel];localStorage.removeItem(r.key);this.removeFromRecent(r.key);this.renderRecentList()};
  });
},
